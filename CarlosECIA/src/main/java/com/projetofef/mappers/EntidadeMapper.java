...
public final class EntidadeMapper {
    ...
    public static EntidadeDTO toDto(Entidade en) {
        if(en == null) return null;

        Integer idDto = en.getId();
        Integer usuarioId = (en.getUsuario() == null) ? null : en.getUsuario().getId();
    ...
    public static Entidade toEntity(EntidadeDTO dto, Usuario usuario) {
        ...
        en.setId(dto.getId());
        en.setNome(trim(dto.getNome()));
        en.setDocumento(trim(dto.getDocumento()));
        en.setTipo(trim(dto.getTipo()));
